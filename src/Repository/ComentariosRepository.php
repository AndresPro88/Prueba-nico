...
class ComentariosRepository extends ServiceEntityRepository
{
    ...
    public function ComentariosUsuario($usuario){
        return $this->getEntityManager()
            ->createQuery('
            SELECT comentarios.fecha_publicacion, post.id, post.titulo
            FROM App:Comentarios comentarios
            JOIN comentarios.post post
            WHERE comentarios.user = 4
            ORDER BY comentarios.fecha_publicacion
            ')->getResult();
    }
